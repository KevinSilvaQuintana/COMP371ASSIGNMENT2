...
Model::Model(Model * parentPointer, bool scaleParent) : mName("UNNAMED"), mPosition(0.0f, 0.0f, 0.0f), mScaling(1.0f, 1.0f, 1.0f), mRotationAxis(0.0f, 1.0f, 0.0f), mRotationAngleInDegrees(0.0f)
{
	this->parentPointer = parentPointer;
	mScaleParent = scaleParent;
}
...
glm::mat4 Model::GetWorldMatrix() const
{
	...
	glm::mat4 translationMatrix = glm::translate(identityMatrix, mPosition);
	glm::mat4 rotationMatrix = glm::rotate(identityMatrix, mRotationAngleInDegrees, mRotationAxis);
	glm::mat4 scalingMatrix = glm::scale(identityMatrix, mScaling);

	glm::mat4 worldMatrix = translationMatrix * rotationMatrix * scalingMatrix;
	...
	if (parentPointer != nullptr){
		...
		if (mScaleParent)
		{
			worldMatrix = parentPointer->GetWorldMatrix() * worldMatrix;
		}
		else
		{
			worldMatrix = parentPointer->GetUnscaledWorldMatrix() * worldMatrix;
		}
	}	
...
glm::mat4 Model::GetUnscaledWorldMatrix() const
{
	glm::mat4 identityMatrix(1.0f);
...
void Model::SetParent(Model * parentPointer){
	this->parentPointer = parentPointer;
}